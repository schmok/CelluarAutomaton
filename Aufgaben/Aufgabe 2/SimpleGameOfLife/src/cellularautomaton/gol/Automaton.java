...
public abstract class Automaton {
    ...
    public Cell[][] calcNextGeneration() {
        ...
        boolean mod = this.isMooreNeighborHood;
        iterator(newCells, (cell, row, col) -> {
            Cell[] neighbors = new Cell[(mod)?8:4];
            int ctr = 0;
            for(int i = 0; i < 9; i++)
                if(i % 2 > ((mod)?-1:0) && i != 4) {
                    int y = i % 3, x = (i / 3);
                    neighbors[ctr++] = new Cell(this.getCell((row-1)+x, (col-1)+y));
                }
            return transform(new Cell(this.getCell(row,col)), neighbors);
        });
        return newCells;
    }
}